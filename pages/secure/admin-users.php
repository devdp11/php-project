...
<div class="p-4 overflow-auto h-100">
    ...
    <div class="row row-cols-1 row-cols-md-3 g-4">
        ...
        <?php foreach ($users as $user) : ?>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col justify-content-center">
                                ...
                                <p class='card-text'><strong>Admin: </strong><?php echo $user['admin'] == 1 ? 'Yes' : 'No'; ?></p>
            ...
            <!-- MODAL EDIT USER -->
            <div class="modal fade" id="edit-user<?= $user['id']; ?>" tabindex="-1" aria-labelledby="edit-user<?= $user['id']; ?>" aria-hidden="true">
                        ...
                        <div class="modal-body pt-0">
                            <form action="../../controllers/admin/user.php" method="post">
                                ...
                                <div class="form-group mt-3">
                                    ...
                                    <input autocomplete="off" type="text" class="form-control" id="first_name" name="first_name" placeholder="First Name" value="<?= isset($user['first_name']) ? $user['first_name'] : '' ?>">
                                </div>
                                <div class="form-group mt-3">
                                    <label>Last Name</label>
                                    <input autocomplete="off" type="text" class="form-control" id="last_name" name="last_name" placeholder="Last Name" value="<?= isset($user['last_name']) ? $user['last_name'] : '' ?>">
                                </div>
                                <div class="form-group mt-3">
                                    <label>Email</label>
                                    <input autocomplete="off" type="email" class="form-control" id="email" name="email" aria-describedby="emailHelp" placeholder="Enter email" value="<?= isset($user['email']) ? $user['email'] : '' ?>">
                                </div>
                                <div class="form-group mt-3">
                                    <label>Birth Date</label>
                                    <input autocomplete="off" type="date" class="form-control" id="birthdate" name="birthdate" aria-describedby="birthDateHelp" placeholder="Enter birth date" value="<?= isset($user['birthdate']) ? $user['birthdate'] : '' ?>">
                                </div>
                                <div class="form-group mt-3">
                                    <label>Country</label>
                                    <select class="form-select" name="country">
                                        <?php foreach ($countries as $country): ?>
                                            <option value="<?= $country['name'] ?>" <?= (isset($user['country']) && $user['country'] == $country['name']) ? 'selected' : '' ?>>
                                                <?= $country['name'] ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                ...
                                <div class="form-check mt-3">
                                    ...
                                    <input autocomplete="off" class="form-check-input" type="checkbox" name="admin" id="admin" <?= isset($user['admin']) && $user['admin'] == 1 ? 'checked' : '' ?>>
                                </div>
                                <button type="submit" class="btn btn-blueviolet mt-3" name="user" value="update">Update</button>
                            </form>
                        </div>
                    </div>
                </div>
